...
def index():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('index.html', result_image=result_path, frames=get_frame_list(), selected_frame=selected_frame)
        except Exception as e:
            logging.error(f"Error during image processing: {e}")
            return jsonify({"error": "Error processing images"}), 500

    return render_template('index.html', result_image=None, frames=get_frame_list(), selected_frame=selected_frame)
...
def process_images(profile_path, template_path):
    ...
    combined_image.paste(template_image, (0, 0)) 
    combined_image.paste(profile_image, (0, 0), profile_image)  
